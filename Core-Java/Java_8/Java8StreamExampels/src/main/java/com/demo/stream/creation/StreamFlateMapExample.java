...
public class StreamFlateMapExample {
	...
	public static void main(String[] args) {
		...
		System.out.println("");
		List<Integer> listNuum2 = listAllNumb.stream().flatMap(List::stream).sorted((i, j) -> j.compareTo(i)).collect(Collectors.toList());
		listNuum2.forEach(System.out::print);
		System.out.println("");
		List<String> listNames = Arrays.asList("Java", "Angular", "spring");
		List<String> listUpdatedNames = listNames.stream().sorted(String.CASE_INSENSITIVE_ORDER).collect(Collectors.toList());
		listUpdatedNames.forEach(System.out::println);
		
		List<String> listNames2 = Arrays.asList("Java", "Angular", "spring");
		List<String> listUpdatedNames2 = listNames2.stream().sorted(String.CASE_INSENSITIVE_ORDER.reversed()).collect(Collectors.toList());
