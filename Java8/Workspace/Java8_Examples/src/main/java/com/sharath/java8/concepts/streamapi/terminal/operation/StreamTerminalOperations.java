package com.sharath.java8.concepts.streamapi.terminal.operation;
...
public class StreamTerminalOperations {
	...
	// element.
	// forEachOrdered(Consumer<? super T> action): Performs the action on elements
	// in encounter order.
	// toArray(): Collects elements into an array.
	// reduce(BinaryOperator<T> accumulator): Reduces the elements of the stream
	// into a single value using the provided function.
	// reduce(T identity, BinaryOperator<T> accumulator): Reduces with an initial
	// value and accumulator.
	// collect(Collector<? super T, A, R> collector): Performs a mutable reduction
	// using a collector (e.g., Collectors.toList()).
	// collect(Supplier<R> supplier, BiConsumer<R, ? super T> accumulator,
	// BiConsumer<R, R> combiner): Another way to collect elements manually.
	// count(): Returns the number of elements in the stream.
	// anyMatch(Predicate<? super T> predicate): Returns true if any element matches
	// the predicate.
	// allMatch(Predicate<? super T> predicate): Returns true if all elements match
	// the predicate.
	// noneMatch(Predicate<? super T> predicate): Returns true if no elements match
	// the predicate.
	...
	public static void main(String[] args) {
		...
		int sum = listNumber.stream().reduce(0, Integer::sum); // 0 indicate start value for doing sum like 0+1 = 1, 1+2
																// = 3 ...
