...
public class StreamCreations {
	...
	// 2. ofNullable(T value): Creates a single-element stream if the value is
	// non-null; otherwise, an empty stream.
	// 3. generate(Supplier<T> s): Creates an infinite stream where each element is
	// generated using the supplier.
	// 4. iterate(T seed, UnaryOperator<T> f): Creates an infinite stream starting
	// with a seed and iteratively applying a function.
